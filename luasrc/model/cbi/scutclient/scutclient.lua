...
-- Resolve the wireless settings URL dynamically so the button works on any
-- device regardless of interface naming (radio0/radio1, network1/wlan, etc.).
-- Falls back to the wireless overview page if no wifi-iface section exists.
local _uci_wlan = require("luci.model.uci").cursor()
local _wifi_url  = luci.dispatcher.build_url("admin/network/wireless")
_uci_wlan:foreach("wireless", "wifi-iface", function(s)
	if s.device and s.network then
		_wifi_url = luci.dispatcher.build_url(
			"admin/network/wireless/" .. s.device .. "." .. s.network)
		return false  -- stop after the first interface
	end
end)
...
scut_option:option(Flag, "enable", translate("启用"))

-- config scutclient
scut_client = scut:section(TypedSection, "scutclient", translate("用户信息"))
scut_client.anonymous = true
scut_client:option(Value, "username", translate("拨号用户名"), translate("学校提供的用户名，一般是学号"))
scut_client:option(Value, "password", translate("拨号密码")).password = true
...
scut_drcom_nettime.description = translate("允许的上网时间，断网后等待到指定时间重新开始认证。如6:15")
...
-- 获取dhcp列表，加入第一个主机名候选
local dhcp_hostnames = string.split(luci.sys.exec("cat /tmp/dhcp.leases 2>/dev/null | awk '{print $4}'"), "\n") or {}

scut_drcom_hostname:value(random_hostname)
if dhcp_hostnames[1] and dhcp_hostnames[1] ~= "" then
