luci-app-scutclient-plus 是 scutclient 的 LuCI 图形化管理界面，专为华南理工大学校园网 DRCOM 认证场景设计。安装后可在 OpenWrt 路由器的 Web 管理页面中完成认证配置、服务启停、实时日志查看及网络状态检测，无需登录 SSH。
...
设置 Wi-Fi：配置接入校园网的无线网卡（仅无线接入场景需要）
设置 IP：在网络→接口中配置 WAN 口 IP，有线接入同理
修改管理密码：强烈建议修改路由器默认密码后再接入校园网
...
Drcom 版本和哈希值是否与所在校区一致——不同接入区域可能使用不同版本
主机名是否符合学校要求（部分接入点会校验主机名格式）
服务器 IP 是否可达：ping 202.38.210.131
在日志页查看实时日志，或下载调试包后检查 scutclient.log
